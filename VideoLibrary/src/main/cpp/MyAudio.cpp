...
//TODO 重点 分包原理，分包pcm数据给app
void *pcmDataCallback(void *data) {
    LOGE("MyAudio pcmDataCallback +");
    MyAudio *audio = static_cast<MyAudio *>(data);
    while (audio->playStatus != nullptr && !audio->playStatus->exit) {
        LOGE("MyAudio pcmDataCallback loop");
        PcmBean *pcmBean = nullptr;
        audio->bufferQueue->getBuffer(&pcmBean);
        LOGE("MyAudio pcmDataCallback getBuffer, pcmBean:%p", pcmBean);
        if (pcmBean == nullptr) {
            LOGE("MyAudio pcmDataCallback pcmBean buffer is null");
            continue;
        }

        LOGD("pcmBean buffer size is %d", pcmBean->bufferSize);
        if (pcmBean->bufferSize <= audio->defaultPcmSize)//不用分包
        {
            if (audio->isRecordPcm) {
                audio->callJava->onCallEncodePCM2AAC(CHILD_THREAD,
                                                     pcmBean->buffer, pcmBean->bufferSize);
            }
            if (audio->isShowPcm) {
                audio->callJava->onCallCutPcmData(CHILD_THREAD,
                                                  pcmBean->buffer, pcmBean->bufferSize);
            }
        } else {
            int pack_num = pcmBean->bufferSize / audio->defaultPcmSize;
            int pack_sub_size = pcmBean->bufferSize % audio->defaultPcmSize;
            LOGD("pcmBean buffer size is %d, pack_num:%d, pack_sub_size:%d",
                 pcmBean->bufferSize, pack_num, pack_sub_size);
            for (int i = 0; i < pack_num; i++) {
                ...
                if (audio->isRecordPcm) {
                    audio->callJava->onCallEncodePCM2AAC(CHILD_THREAD, bf, audio->defaultPcmSize);
                }
                if (audio->isShowPcm) {
                    audio->callJava->onCallCutPcmData(CHILD_THREAD, bf, audio->defaultPcmSize);
                }
                free(bf);
                bf = nullptr;
            }
            ...
            if (pack_sub_size > 0) {
                char *bf = static_cast<char *>(malloc(pack_sub_size));
                memcpy(bf, pcmBean->buffer + pack_num * audio->defaultPcmSize, pack_sub_size);
                if (audio->isRecordPcm) {
                    audio->callJava->onCallEncodePCM2AAC(CHILD_THREAD, bf, pack_sub_size);
                }
                if (audio->isShowPcm) {
                    audio->callJava->onCallCutPcmData(CHILD_THREAD, bf, pack_sub_size);
                }
                free(bf);
                bf = nullptr;
            }
    ...
    LOGE("MyAudio pcmDataCallback -");
//    return nullptr;//todo must return
    pthread_exit(&audio->pid_pcmcallback);
}
...
void MyAudio::play() {
    ...
    //todo estimate app set condition to Decide whether to do
    pthread_create(&pid_pcmcallback, nullptr, pcmDataCallback, this);
    LOGE("MyAudio::play -");
}
...
//            LOGE("MyAudio::resampleAudio avcodec_receive_frame ret:%d",ret);
            ...
            LOGE("MyAudio resampleAudio channels:%d, channel_layout:%ld", avFrame->channels,
...
//                myAudio->callJava->onCallEncodePCM2AAC(CHILD_THREAD, bufferSize * 2 * 2,
//                                                       myAudio->sampleBuffer);
//            }
            //TODO 把PCM数据放到bufferQueue中
            myAudio->bufferQueue->putBuffer(myAudio->sampleBuffer, bufferSize * 2 * 2);
...
void MyAudio::release() {
    if (bufferQueue != nullptr) {
        bufferQueue->release();
        //TODO 阻塞这里等待线程结束再继续执行
        pthread_join(pid_pcmcallback, nullptr);
        delete (bufferQueue);
        bufferQueue = nullptr;
    }

    if (queue != nullptr) {//TODO need clear and pthread_join?
        delete (queue);
        queue = nullptr;
    }
    ...
    //TODO need clear and pthread_join?
